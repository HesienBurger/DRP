...
[] call life_fnc_soundloop;
 ...
 [] spawn {
	 ...
	 while {true} do {
		 ...
		 if(_server != "Arma 3 DefianceLifeRP" or _channel != "TaskForceRadio") then {
			
			 cutText["Wrong TS Channel.","BLACK",0,true];
			 waitUntil {([] call TFAR_fnc_getTeamSpeakServerName) == "Arma 3 DefianceLifeRP" and ([] call life_fnc_getTeamSpeakChannelName) == "TaskForceRadio"};
			 cutText["","PLAIN"];
