...
[] call life_fnc_soundloop;
 ...
 [] spawn {
	 waitUntil {life_session_completed};
	 while {true} do {
		 _server = [] call life_fnc_getTeamSpeakServerName;
		 _channel = [] call life_fnc_getTeamSpeakChannelName;

         if(!([] call life_fnc_isTeamSpeakPluginEnabled)) then {
        
        	 cutText["Enable TFR Plugin on Teamspeak!","BLACK"];
			 waitUntil {([] call life_fnc_isTeamSpeakPluginEnabled)};
			 cutText["","PLAIN"];
			
         };

		 if(_server != "Deluxe Life Roleplay Arma 3 Life" or _channel != "TaskForceRadio") then {
			
			 cutText["Wrong TS Channel.","BLACK",0,true];
			 waitUntil {([] call TFAR_fnc_getTeamSpeakServerName) == "Deluxe Life Roleplay Arma 3 Life" and ([] call life_fnc_getTeamSpeakChannelName) == "TaskForceRadio"};
			 cutText["","PLAIN"];
